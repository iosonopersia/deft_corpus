...
def write_converted(source_file, output_file):
    ...
    with open(source_file, mode="rt", encoding="utf-8") as source_text:
        has_def = 0
        new_sentence = ''
        ...
        for index, line in enumerate(all_lines):
            ...
            if re.match('^\s+$', line) and len(new_sentence) > 0 and (not re.match(r'^\s*\d+\s*\.$', new_sentence)
                ...
                sentences = pd.concat([sentences, pd.DataFrame([{'sentence': new_sentence, 'label': has_def}])], ignore_index=True)
                new_sentence = ''
                has_def = 0
