...
def train_loop():
    ...
    train_data = DefinitionsFactsDataset(TRAIN_PATH, tokenizer, max_sequence_length=MAX_LENGTH)
    eval_data = DefinitionsFactsDataset(VAL_PATH, tokenizer, max_sequence_length=MAX_LENGTH)
    train_loader = torch.utils.data.DataLoader(train_data, **dataloader_cfg.train)
    eval_loader = torch.utils.data.DataLoader(eval_data, **dataloader_cfg.val)

    # ===========MODEL===============
    model = RoBERTaSentenceClassifier(len(tokenizer), **model_cfg)
    model = model.to(DEVICE)
    loss_fn = CustomBCELoss(class_weights=train_data.get_class_weights(), label_smoothing=LABEL_SMOOTHING)
...
if __name__ == "__main__":
    ...
    # ===========DATASET==============
    TRAIN_PATH = dataset_cfg.train_labels_file
    VAL_PATH = dataset_cfg.val_labels_file
    MAX_LENGTH = dataset_cfg.max_sentence_length

    # ========HYPERPARAMETERS========
    LABEL_SMOOTHING = hyperparams_cfg.label_smoothing
